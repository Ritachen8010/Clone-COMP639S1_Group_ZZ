...
CREATE TABLE lesson_schedule (
    `lesson_schedule_id` INT PRIMARY KEY AUTO_INCREMENT,
    `instructor_id` INT,
    `description` TEXT,
    `start_time` TIME NOT NULL,
    `end_time` TIME NOT NULL,
...
CREATE TABLE bookings (
    ...
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    FOREIGN KEY (lesson_schedule_id) REFERENCES lesson_schedule(lesson_schedule_id) ON DELETE CASCADE,
    FOREIGN KEY (class_schedule_id) REFERENCES class_schedule(class_schedule_id) ON DELETE CASCADE
);
